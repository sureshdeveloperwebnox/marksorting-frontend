...
export default function TicketsPage() {
  ...
  // Shared context filters applied to tab counts (respects date + other filters)
  ...
  const { data: totalData, refetch: refetchTotal, isFetching: isFetchingTotal } = useTickets({ skip: 0, take: 1, ...sharedCountFilters });
  const { data: openData, refetch: refetchOpen, isFetching: isFetchingOpen } = useTickets({ skip: 0, take: 1, ...sharedCountFilters, status: "OPEN" });
  const { data: inProgressData, refetch: refetchInProgress, isFetching: isFetchingInProgress } = useTickets({ skip: 0, take: 1, ...sharedCountFilters, status: "IN_PROGRESS" });
  const { data: resolvedData, refetch: refetchResolved, isFetching: isFetchingResolved } = useTickets({ skip: 0, take: 1, ...sharedCountFilters, status: "RESOLVED" });
  const { data: escalatedData, refetch: refetchEscalated, isFetching: isFetchingEscalated } = useTickets({ skip: 0, take: 1, ...sharedCountFilters, status: "ESCALATED" });
  ...
  const activeFilterCount = [statusFilter, priorityFilter, dateFrom, dateTo].filter(Boolean).length;
