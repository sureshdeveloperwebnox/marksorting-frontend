...
export function AntigravityText({
  ...
  // Set default scatter range based on content type if not specified
  const actualScatterRange = scatterRange ?? (useSvgLogo ? 500 : 150);

  React.useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

    // Find all targets to animate (either children of SVG, or individual text spans)
    const targets = useSvgLogo
      ? container.querySelectorAll('svg path, svg polygon')
      : container.querySelectorAll('.antigravity-char');
    ...
    // Set initial random scatter positions and rotations
    targets.forEach((target) => {
      gsap.set(target, {
        x: `+=${getRandom(-actualScatterRange, actualScatterRange)}`,
        y: `+=${getRandom(-actualScatterRange, actualScatterRange)}`,
        rotation: `+=${getRandom(-rotationRange, rotationRange)}`,
        scale: 0,
        opacity: 0,
      });
    });

    // Animate to original position (assemble)
    tl.to(
      targets,
      {
        x: 0,
  ...
  const handleMouseEnter = () => {
    if (interactive && timelineRef.current) {
  ...
  // If using the SVG Logo "ANTIGRAVITY" spelling
  if (useSvgLogo) {
    return (
      <div
        ref={containerRef}
        ...
        className={`flex items-center justify-center overflow-visible select-none cursor-pointer ${className}`}
      >
        <svg
          x="0px"
          y="0px"
          ...
          style={{ fill: color, stroke: color, strokeWidth: '0.85px' }}
        >
  ...
  // If animating standard text characters
  const charArray = text ? text.split('') : [];
  ...
  return (
    <div
      ...
      style={{
        fontFamily: 'var(--font-barlow-condensed), "Barlow Condensed", "Arial Black", "Impact", sans-serif',
        fontWeight: 900,
        fontStyle: 'normal',
    ...
    >
      ...
      <span aria-hidden="true" className="flex flex-wrap items-center">
        {charArray.map((char, index) => {
          const isSpace = char === ' ';
          return (
            <span
              key={index}
              className="antigravity-char inline-block origin-center transform-gpu"
              style={{
                whiteSpace: 'pre',
