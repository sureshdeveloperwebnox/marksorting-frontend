...
import { useMills, useMill } from '@/services/mill-service';
import { cn } from '@/lib/utils';

interface MillSearchSelectProps {
  value: string;
...
export function MillSearchSelect({
  value,
  onChange,
  placeholder = 'Select a mill...',
  disabled = false,
}: MillSearchSelectProps) {
  const [open, setOpen] = React.useState(false);
  const [search, setSearch] = React.useState('');
  const containerRef = React.useRef<HTMLDivElement>(null);
  const searchRef = React.useRef<HTMLInputElement>(null);

  const { data, isLoading } = useMills({ skip: 0, take: 1000, status: 'ACTIVE' });
  const mills = data?.mills || [];

  const filtered = mills.filter((m) =>
    m.name.toLowerCase().includes(search.toLowerCase()) ||
    (m.ref_no || '').toLowerCase().includes(search.toLowerCase()) ||
  ...
  const selectedMill = mills.find((m) => m.id === value);

  // Fallback: fetch by ID when editing a record whose mill isn't in the ACTIVE list
  const { data: fallbackMill } = useMill(
    value && !selectedMill && !isLoading ? value : null
  );

  // Resolved display name
  const displayName = selectedMill?.name ?? fallbackMill?.name ?? null;
      ...
      >
        {displayName ? (
          ...
          <span className="text-gray-400 dark:text-gray-600 text-sm font-medium">
            {isLoading ? 'Loading mills...' : placeholder}
          </span>
        )}
          ...
          <div className="p-2 border-b border-gray-100 dark:border-white/5">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search by mill, customer, ref no, or place..."
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-6 gap-1.5 text-gray-400">
                ...
                <span className="text-xs font-semibold">
                  {search ? 'No mills match your search' : 'No mills available'}
                </span>
              </div>
            ) : (
              filtered.map((mill) => {
                const selected = value === mill.id;
                return (
                  <button
                    key={mill.id}
                    type="button"
                    onClick={() => select(mill.id)}
                    className={cn(
                  ...
                  >
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-primary/10 to-primary/5 dark:from-white/10 dark:to-white/5 flex items-center justify-center text-primary font-bold text-xs border border-primary/10 flex-shrink-0">
                      {mill.name.charAt(0).toUpperCase()}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        {mill.ref_no && (
                          <span className="text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 px-1 py-0.5 rounded font-mono">
                            {mill.ref_no}
                          </span>
                        )}
                        {mill.customer?.name && (
